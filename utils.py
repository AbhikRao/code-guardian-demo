...
def get_env_secret():
    ...
    if secret is None:
        raise ValueError("SECRET_KEY environment variable is not set")  # FIXED: removed hardcoded secret
    return secret

def process_items(items):
    """Process a list of items."""
    # FIXED: created a new list instead of modifying the original list
    return [item for item in items if item >= 0]

def read_all_lines(filepath):
    """Read lines from file."""
    with open(filepath, "r") as f:  # FIXED: used context manager for file handle
        return f.readlines()
